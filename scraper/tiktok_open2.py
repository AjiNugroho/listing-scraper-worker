...
class TikTokScraperNoLogin:
    ...
    async def setup_xhr_interception(self, page: Page):
        """Set up XHR interception for API responses on specific page"""
        ...
        # Intercept all requests matching the pattern on the specific page
        await page.route("**/*", handle_route)
        print("✓ XHR interception enabled on new tab\n")
    ...
    async def extract_video_urls(self, page: Page) -> Set[str]:
    ...
    async def scroll_and_collect(self, url: str, max_items: int = 50, scroll_delay: int = 2):
        ...
        print("→ Opening new tab for target URL...")
        ...
        # Now enable XHR interception on the new tab
        await self.setup_xhr_interception(new_page)
        
        # Handle any popups
        # try:
        #     close_button = await new_page.query_selector('button[aria-label="Close"]')
        #     if close_button:
        #         await close_button.click()
        ...
        while len(self.collected_urls) < max_items:
            ...
            new_urls = await self.extract_video_urls(new_page)
            ...
            # Smooth scroll down
            await self.smooth_scroll(new_page)
            
            # Wait before next scroll
            print(f"  ⏳ Waiting {scroll_delay} seconds before next scroll...")
            await new_page.wait_for_timeout(scroll_delay * 1000)
