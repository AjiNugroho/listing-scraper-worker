...
class TikTokScraper:
    def __init__(self, username: str = None, password: str = None, cookies_file: str = "tiktok_cookies.json"):
    ...
    async def is_logged_in(self) -> bool:
        ...
        try:
            ...
            login_button = await self.page.query_selector('button[data-e2e="top-login-button"]')
    ...
    async def login(self):
        """
        Automatic login to TikTok
        """
        if not self.username or not self.password:
            raise Exception("Username and password are required for automatic login")
        ...
        try:
            ...
            await self.page.goto('https://www.tiktok.com/login/phone-or-email/email', wait_until='domcontentloaded')
            ...
            # # Handle potential popups/prompts
            # # Handle "Get the TikTok app" prompt if appears
            # try:
            #     get_app_close = await self.page.query_selector('button[aria-label="Close"]')
            #     if get_app_close:
            #         print("→ Closing 'Get App' prompt...")
            #         await get_app_close.click()
            #         await asyncio.sleep(2)
            # except:
            #     pass
            ...
            if await self.is_logged_in():
                ...
                print("Note: TikTok may require manual verification (CAPTCHA, email code, etc.)")
                print("If this happens frequently, consider using manual login mode.")
                return False
                
        except Exception as e:
            print(f"\n✗ Login error: {e}")
            print("Tip: TikTok has strong bot detection. You may need to:")
            print("  1. Use a different IP address")
            print("  2. Add human-like delays")
            print("  3. Complete manual verification if prompted")
            return False
...
async def main():
    """Example usage"""
    # Set your TikTok credentials here
    USERNAME = "aji.nugroho@fair-indonesia.com"  # or username
    PASSWORD = "ajiGGtiktok@fair123"
    
    scraper = TikTokScraper(username=USERNAME, password=PASSWORD)
    ...
    try:
        ...
        # Example 1: Scroll and collect URLs from a search or profile page
